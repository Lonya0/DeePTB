...
from dptb.utils import instantiate, get_w_prefix, Config
...
def dataset_from_config(config: Union[dict, Config], prefix: str = "dataset") -> AtomicDataset:
    ...
    config : Union[dict, Config]
        dict/object that store all the parameters
    prefix : str, default="dataset"
        The prefix of all dataset parameters
...
class DatasetBuilder:
    """This class is used to perform as function build_dataset()
    You can see it from __call__ function comment.
    """
    def __init__(self):
        pass
    ...
    def __call__(self,
                 ...
                 basis: str = None,
                 **kwargs,
                 ) -> Union[DeePHE3Dataset, DefaultDataset, HDF5Dataset, LMDBDataset]:

        """Build a dataset based on the provided set options and common options.

        Logic
        -----
        1. Validate input parameters and dataset type.
        2. Get an OrbitalMapper based on ``basis``.
        3. Collect trajectory folders using ``prefix`` and ``root``.
        4. Load and process ``info.json`` metadata (public or per-folder).
        5. Initialize specific dataset type with orbital mappings and data loading flags.

        Parameters
        ----------
        root : str
            The main directory storing all trajectory folders.
        r_max : Union[float, int, dict]
            Cutoff radius for atomic interactions (global or atom-pair specific).
        er_max : Union[float, int, dict], default = None
            Environment-related cutoff.
        oer_max : Union[float, int, dict], default = None
            Orbital-environment cutoff.
        type : str, default = "DefaultDataset"
            The type of dataset to build.
        prefix : str, optional
            Load selected trajectory folders with the specified prefix.
        separator: str, default = '.'
            Prefix-folder separator.
        get_Hamiltonian : bool, default=False
            Load the Hamiltonian file to edges of the graph or not.
        get_overlap : bool, default=False
            Load the overlap file to edges of the graph or not.
        get_DM : bool, default=False
            Load the density matrix file to edges of the graph or not.
        get_eigenvalues : bool, default=False
            Load the eigenvalues to the graph or not.
        orthogonal : bool, default=False
            Orthogonality flag (for LMDBDataset).
        basis : str, optional
            The basis for the OrbitalMapper.
        ...
        -------
        Union[DeePHE3Dataset, DefaultDataset, HDF5Dataset, LMDBDataset]
            The built dataset. Type is defined by input parameter 'type'.

        Raises
        ------
        AssertionError
            1) If prefix is not provided.
            2) If the dataset type is not supported.
        ValueError
            for {dataset_type} type, the info.json is not properly provided for `{file}`
        Exception
            If the info.json file is not properly provided for a trajectory folder.
        """
        self.r_max = r_max
        self.er_max = er_max
        ...
        assert dataset_type in ["DefaultDataset", "DeePHDataset", "HDF5Dataset", "LMDBDataset"], (f"Not support dataset "
                                                                                                  f"type: {type}.")
        assert prefix is not None, ("The prefix is not provided. Please provide the prefix to select the trajectory "
                                    "folders.")
        ...
        for idir in prefix_folders:
            if os.path.isdir(idir):
                if not glob.glob(os.path.join(idir, '*.dat')) \
                    ...
                    raise Exception(
                        f"{idir} does not have the proper traj data files. Please check the data files.")
                include_folders.append(idir.split('/')[-1])

        assert isinstance(include_folders, list) and len(
            include_folders) > 0, "No trajectory folders are found. Please check the prefix."
        ...
        if os.path.exists(f"{root}/info.json"):
            ...
            if dataset_type == "LMDBDataset":
                ...
                log.info(
                    "A public `info.json` file is provided, and will be used by the subfolders who do not have their own `info.json` file.")
        else:
            public_info = None
        ...
        if model is None:
            ...
            if isinstance(cutoff_options['r_max'], dict):
                ...
                for key in cutoff_options['r_max']:
                    ...
                    assert self.r_max >= cutoff_options['r_max'][
                        key], f"The r_max in model shoule be  smaller than in dataset for {key}."
            ...
            if isinstance(cutoff_options['er_max'], dict):
                ...
                for key in cutoff_options['er_max']:
                    ...
                    assert self.er_max >= cutoff_options['er_max'][
                        key], f"The er_max in model shoule be  smaller than in dataset for {key}."
                ...
                assert self.er_max >= cutoff_options[
                    'er_max'], "The er_max in model shoule be  smaller than in dataset."
            ...
            if isinstance(cutoff_options['oer_max'], dict):
                ...
                for key in cutoff_options['oer_max']:
                    ...
                    assert self.oer_max >= cutoff_options['oer_max'][
                        key], f"The oer_max in model shoule be  smaller than in dataset for {key}."
                ...
                assert self.oer_max >= cutoff_options[
                    'oer_max'], "The oer_max in model shoule be  smaller than in dataset."
